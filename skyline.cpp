...
// 解题思路：
// 从南北方向看，每一列的最大高度为转置矩阵的每一行中最大元素max_t，而从东西方向看最大高度为原矩阵的每一行最大元素max，遍历原矩阵行，只需要使得其它元素增加值不超过min(max,
// max_t)，就能得到最大增量值。
#include <algorithm>
#include <array>
#include <iostream>
...
int maxIncreaseKeepingSkyline(vector< vector< int > > &grid)
{
    ...
    vector< int > max_element_row_t(row);
    for (int i = 0; i < row; ++i)
    {
        int row_max_t =
            *std::max_element(transpose[i].begin(), transpose[i].end());
        max_element_row_t.at(i) = row_max_t;
    }

    for (int i = 0; i < row; ++i)
    {
        int row_max = *std::max_element(grid[i].begin(), grid[i].end());

        for (int j = 0; j < col; ++j)
        {
            int max = std::min(row_max, max_element_row_t[j]);
            int elem = grid[i][j];
            if (elem < max)
            {
                ans += (max - elem);
